...
if REPENTOGON then
	function Menorah:MultiShopUpdate(player)
		...
		if data.MenorahFlames and data.MenorahFlames > 1 and player:HasCollectible(AntibirthItemPack.CollectibleType.COLLECTIBLE_MENORAH) then
	...
	AntibirthItemPack:AddCallback(ModCallbacks.MC_POST_PLAYER_GET_MULTI_SHOT_PARAMS, Menorah.MultiShopUpdate)
else
	...
	AntibirthItemPack:AddCallback(ModCallbacks.MC_POST_FIRE_TEAR, Menorah.postFireTear)
	AntibirthItemPack:AddCallback(ModCallbacks.MC_POST_LASER_UPDATE, Menorah.onLaserUpdate)
	AntibirthItemPack:AddCallback(ModCallbacks.MC_POST_BOMB_UPDATE, Menorah.onBombUpdate)
end
...
AntibirthItemPack:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, Menorah.onEvaluateCache)
AntibirthItemPack:AddCallback(ModCallbacks.MC_FAMILIAR_INIT, Menorah.onFamiliarInit, FamiliarVariant.MENORAH)
AntibirthItemPack:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, Menorah.onFamiliarUpdate, FamiliarVariant.MENORAH)
AntibirthItemPack:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, Menorah.onDamage, EntityType.ENTITY_PLAYER)